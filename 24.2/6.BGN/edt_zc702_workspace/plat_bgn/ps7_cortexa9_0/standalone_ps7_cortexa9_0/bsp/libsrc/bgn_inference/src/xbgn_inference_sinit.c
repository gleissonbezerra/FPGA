// ==============================================================
// Vitis HLS - High-Level Synthesis from C, C++ and OpenCL v2024.2 (64-bit)
// Tool Version Limit: 2024.11
// Copyright 1986-2022 Xilinx, Inc. All Rights Reserved.
// Copyright 2022-2024 Advanced Micro Devices, Inc. All Rights Reserved.
// 
// ==============================================================
#ifndef __linux__

#include "xstatus.h"
#ifdef SDT
#include "xparameters.h"
#endif
#include "xbgn_inference.h"

extern XBgn_inference_Config XBgn_inference_ConfigTable[];

#ifdef SDT
XBgn_inference_Config *XBgn_inference_LookupConfig(UINTPTR BaseAddress) {
	XBgn_inference_Config *ConfigPtr = NULL;

	int Index;

	for (Index = (u32)0x0; XBgn_inference_ConfigTable[Index].Name != NULL; Index++) {
		if (!BaseAddress || XBgn_inference_ConfigTable[Index].Ctrl_BaseAddress == BaseAddress) {
			ConfigPtr = &XBgn_inference_ConfigTable[Index];
			break;
		}
	}

	return ConfigPtr;
}

int XBgn_inference_Initialize(XBgn_inference *InstancePtr, UINTPTR BaseAddress) {
	XBgn_inference_Config *ConfigPtr;

	Xil_AssertNonvoid(InstancePtr != NULL);

	ConfigPtr = XBgn_inference_LookupConfig(BaseAddress);
	if (ConfigPtr == NULL) {
		InstancePtr->IsReady = 0;
		return (XST_DEVICE_NOT_FOUND);
	}

	return XBgn_inference_CfgInitialize(InstancePtr, ConfigPtr);
}
#else
XBgn_inference_Config *XBgn_inference_LookupConfig(u16 DeviceId) {
	XBgn_inference_Config *ConfigPtr = NULL;

	int Index;

	for (Index = 0; Index < XPAR_XBGN_INFERENCE_NUM_INSTANCES; Index++) {
		if (XBgn_inference_ConfigTable[Index].DeviceId == DeviceId) {
			ConfigPtr = &XBgn_inference_ConfigTable[Index];
			break;
		}
	}

	return ConfigPtr;
}

int XBgn_inference_Initialize(XBgn_inference *InstancePtr, u16 DeviceId) {
	XBgn_inference_Config *ConfigPtr;

	Xil_AssertNonvoid(InstancePtr != NULL);

	ConfigPtr = XBgn_inference_LookupConfig(DeviceId);
	if (ConfigPtr == NULL) {
		InstancePtr->IsReady = 0;
		return (XST_DEVICE_NOT_FOUND);
	}

	return XBgn_inference_CfgInitialize(InstancePtr, ConfigPtr);
}
#endif

#endif

