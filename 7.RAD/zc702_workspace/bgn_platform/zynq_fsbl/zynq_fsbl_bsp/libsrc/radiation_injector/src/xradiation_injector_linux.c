...
typedef struct {
    ...
    XRadiation_injector_uio_map maps[ MAX_UIO_MAPS ];
} XRadiation_injector_uio_info;

/***************** Variable Definitions **************************************/
static XRadiation_injector_uio_info uio_info;
...
static int uio_info_read_name(XRadiation_injector_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XRadiation_injector_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XRadiation_injector_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XRadiation_injector_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XRadiation_injector_Initialize(XRadiation_injector *InstancePtr, const char* InstanceName) {
	XRadiation_injector_uio_info *InfoPtr = &uio_info;
