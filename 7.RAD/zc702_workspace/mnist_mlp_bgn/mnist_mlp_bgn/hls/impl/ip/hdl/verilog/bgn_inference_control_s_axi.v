...
// ==============================================================
`timescale 1ns/1ps
(* DowngradeIPIdentifiedWarnings="yes" *) module bgn_inference_control_s_axi
#(parameter
...
localparam
    ...
    ADDR_IER               = 8'h08,
    ADDR_ISR               = 8'h0c,
    ADDR_WEIGHT_MEM_DATA_0 = 8'h10,
    ADDR_WEIGHT_MEM_DATA_1 = 8'h14,
    ADDR_WEIGHT_MEM_CTRL   = 8'h18,
    ADDR_PREDICTION_DATA_0 = 8'h1c,
    ADDR_PREDICTION_CTRL   = 8'h20,
...
//------------------------Instantiation------------------
// int_input_img
bgn_inference_control_s_axi_ram #(
    .MEM_STYLE  ( "auto" ),
    .MEM_TYPE   ( "2P" ),
    .BYTE_WIDTH ( 8 ),
    .WIDTH      ( 32 ),
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_WEIGHT_MEM_DATA_0: begin
                    rdata <= int_weight_mem[31:0];
                end
                ADDR_WEIGHT_MEM_DATA_1: begin
                    rdata <= int_weight_mem[63:32];
                end
...
assign weight_mem        = int_weight_mem;
// int_interrupt
always @(posedge ACLK) begin
    if (ARESET)
        int_interrupt <= 1'b0;
...
always @(posedge ACLK) begin
    if (ARESET)
        int_weight_mem[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_WEIGHT_MEM_DATA_0)
            int_weight_mem[31:0] <= (WDATA[31:0] & wmask) | (int_weight_mem[31:0] & ~wmask);
    end
end

// int_weight_mem[63:32]
always @(posedge ACLK) begin
    if (ARESET)
        int_weight_mem[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_WEIGHT_MEM_DATA_1)
            int_weight_mem[63:32] <= (WDATA[31:0] & wmask) | (int_weight_mem[63:32] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (prediction_ap_vld)
            int_prediction <= prediction;
    end
end

// int_prediction_ap_vld
always @(posedge ACLK) begin
    if (ARESET)
        int_prediction_ap_vld <= 1'b0;
    else if (ACLK_EN) begin
        if (prediction_ap_vld)
...
assign int_input_img_address1 = ar_hs ? raddr[6:2] : waddr[6:2];
assign int_input_img_ce1      = ar_hs | (int_input_img_write & WVALID);
assign int_input_img_we1      = int_input_img_write & w_hs;
assign int_input_img_be1      = int_input_img_we1 ? WSTRB : 4'd0;
...
module bgn_inference_control_s_axi_ram
#(parameter
    MEM_STYLE  = "auto",
    MEM_TYPE   = "S2P",
    BYTE_WIDTH = 8,
