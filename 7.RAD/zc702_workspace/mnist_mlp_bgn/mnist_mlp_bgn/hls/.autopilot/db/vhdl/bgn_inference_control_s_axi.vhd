...
entity bgn_inference_control_s_axi is
generic (
...
end entity bgn_inference_control_s_axi;
...
architecture behave of bgn_inference_control_s_axi is
...
attribute DowngradeIPIdentifiedWarnings of behave : architecture is "yes";
    ...
    constant ADDR_WEIGHT_MEM_DATA_0 : INTEGER := 16#10#;
    constant ADDR_WEIGHT_MEM_DATA_1 : INTEGER := 16#14#;
    constant ADDR_WEIGHT_MEM_CTRL   : INTEGER := 16#18#;
    constant ADDR_PREDICTION_DATA_0 : INTEGER := 16#1c#;
    constant ADDR_PREDICTION_CTRL   : INTEGER := 16#20#;
    ...
    signal int_weight_mem      : UNSIGNED(63 downto 0) := (others => '0');
    ...
    signal int_input_img_q0    : UNSIGNED(31 downto 0);
    signal int_input_img_address1 : UNSIGNED(4 downto 0);
    signal int_input_img_ce1   : STD_LOGIC;
    signal int_input_img_be1   : UNSIGNED(3 downto 0);
    signal int_input_img_we1   : STD_LOGIC;
    signal int_input_img_d1    : UNSIGNED(31 downto 0);
    signal int_input_img_q1    : UNSIGNED(31 downto 0);
    signal int_input_img_read  : STD_LOGIC;
    signal int_input_img_write : STD_LOGIC;

    component bgn_inference_control_s_axi_ram is
        generic (
    ...
    end component bgn_inference_control_s_axi_ram;
...
-- ----------------------- Instantiation------------------
-- int_input_img
int_input_img : bgn_inference_control_s_axi_ram
generic map (
     MEM_STYLE  => "auto",
     MEM_TYPE   => "2P",
     BYTE_WIDTH => 8,
...
-- ----------------------- AXI READ ----------------------
    ...
    RRESP   <= "00";  -- OKAY
    RVALID_t  <= '1' when (rstate = rddata) and (int_input_img_read = '0') else '0';
    RVALID    <= RVALID_t;
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_WEIGHT_MEM_DATA_0 =>
                        rdata_data <= RESIZE(int_weight_mem(31 downto 0), 32);
                    when ADDR_WEIGHT_MEM_DATA_1 =>
                        rdata_data <= RESIZE(int_weight_mem(63 downto 32), 32);
                ...
                elsif (int_input_img_read = '1') then
                    rdata_data <= RESIZE(int_input_img_q1, 32);
                end if;
            end if;
        end if;
    end process;
...
-- ----------------------- Register logic ----------------
    ...
    weight_mem           <= STD_LOGIC_VECTOR(int_weight_mem);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_weight_mem(31 downto 0) <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_WEIGHT_MEM_DATA_0) then
                    int_weight_mem(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_weight_mem(31 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_weight_mem(63 downto 32) <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_WEIGHT_MEM_DATA_1) then
                    int_weight_mem(63 downto 32) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_weight_mem(63 downto 32));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_prediction <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (prediction_ap_vld = '1') then
                    int_prediction <= UNSIGNED(prediction);
                end if;
            end if;
        end if;
    end process;
...
-- ----------------------- Memory logic ------------------
    -- input_img
    int_input_img_address0 <= RESIZE(UNSIGNED(input_img_address0), 5);
    int_input_img_ce0    <= input_img_ce0;
    input_img_q0         <= STD_LOGIC_VECTOR(RESIZE(int_input_img_q0, 32));
    int_input_img_address1 <= raddr(6 downto 2) when ar_hs = '1' else waddr(6 downto 2);
    int_input_img_ce1    <= '1' when ar_hs = '1' or (int_input_img_write = '1' and WVALID  = '1') else '0';
    int_input_img_we1    <= '1' when int_input_img_write = '1' and w_hs = '1' else '0';
    int_input_img_be1    <= UNSIGNED(WSTRB) when int_input_img_we1 = '1' else (others=>'0');
    int_input_img_d1     <= UNSIGNED(WDATA);

    process (ACLK)
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_input_img_read <= '0';
            elsif (ACLK_EN = '1') then
                if (ar_hs = '1' and raddr >= ADDR_INPUT_IMG_BASE and raddr <= ADDR_INPUT_IMG_HIGH) then
                    int_input_img_read <= '1';
                else
                    int_input_img_read <= '0';
                end if;
            end if;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_input_img_write <= '0';
            elsif (ACLK_EN = '1') then
                if (aw_hs = '1' and UNSIGNED(AWADDR(ADDR_BITS-1 downto 0)) >= ADDR_INPUT_IMG_BASE and UNSIGNED(AWADDR(ADDR_BITS-1 downto 0)) <= ADDR_INPUT_IMG_HIGH) then
                    int_input_img_write <= '1';
                elsif (w_hs = '1') then
                    int_input_img_write <= '0';
                end if;
            end if;
...
entity bgn_inference_control_s_axi_ram is
    generic (
        MEM_STYLE  : STRING :="auto";
        MEM_TYPE   : STRING :="S2P";
        BYTE_WIDTH : INTEGER :=8;
...
end entity bgn_inference_control_s_axi_ram;

architecture behave of bgn_inference_control_s_axi_ram is
